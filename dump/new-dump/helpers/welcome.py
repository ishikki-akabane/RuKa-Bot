...
async def initialise_welcome(template_id, client, user, chat):
    ...
    if data["data"]["user_pfp"]:
        ...
        userphoto = await client.download_media(user_pfp, file_name=f"../downloads/{user.id}userpfp.jpg")
    ...
    if data["data"]["chat_pfp"]:
        ...
        chatphoto = await client.download_media(chat_pfp, file_name=f"../downloads/{user.id}chatpfp.jpg")
