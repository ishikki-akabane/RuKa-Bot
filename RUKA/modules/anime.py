...
def ANItime(milliseconds: int) -> str:
...
async def character(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if json:
        ...
        if image:
            ...
            await message.reply_photo(photo=image, caption=msg, parse_mode=ParseMode.MARKDOWN)
        else:
            await message.reply_text(msg, parse_mode=ParseMode.MARKDOWN)
...
AIRING_HANDLER = DisableCommandHandler("airing", airing, block=False)
ANIME_HANDLER = DisableCommandHandler("anime", anime, block=False)
CHARACTER_HANDLER = DisableCommandHandler("character", character, block=False)
MANGA_HANDLER = DisableCommandHandler("manga", manga, block=False)

dp.add_handler(AIRING_HANDLER)
