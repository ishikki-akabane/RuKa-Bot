...
from Sumi import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER, SUPPORT_CHAT
...
# from Sumi.modules.rules import get_rules
import Sumi.modules.sql.rules_sql as rulessql

# from Sumi.modules.sql import warns_sql as warnssql
import Sumi.modules.sql.blacklist_sql as blacklistsql
from Sumi.modules.sql import disable_sql as disabledsql

# from Sumi.modules.sql import cust_filters_sql as filtersql
# import Sumi.modules.sql.welcome_sql as welcsql
...
def export_data(update: Update, context: CallbackContext):
    ...
    with open("Sumi{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Sumi{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Sumi-Backup` was specially made for notes.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("Sumi{}.backup".format(chat_id))  # Cleaning file
